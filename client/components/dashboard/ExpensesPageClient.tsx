...
export function ExpensesPageClient() {
  ...
  const expensesQuery = useGetExpensesQuery({ token: token || "" }, { skip: !token });
  const [updateExpense, updateExpenseMutation] = useUpdateExpenseMutation();
  const [deleteExpense, deleteExpenseMutation] = useDeleteExpenseMutation();
  const expenses = expensesQuery.data?.expenses || [];

  const categories = useMemo(
    () => ["All", ...new Set(expenses.map((expense) => expense.category).filter(Boolean))],
    [expenses]
  );
  ...
  const filteredExpenses = useMemo(() => {
    return [...expenses]
      ...
      .filter((expense) => {
        ...
        return getMonthKey(expense.date) === selectedMonth;
      })
      .sort((left, right) =>
        amountSort === "asc" ? left.amount - right.amount : right.amount - left.amount
      );
  }, [amountSort, expenses, selectedCategory, selectedMonth]);
  ...
  async function handleUpdateExpense(event: FormEvent<HTMLFormElement>, expense: ExpenseItem) {
    ...
    const payload: {
      ...
      date: String(formData.get("date") || formatDateInputValue(expense.date)),
    };

    try {
      if (receiptFile instanceof File && receiptFile.size > 0) {
        setUploadingExpenseId(expense._id);
      ...
      setExpenseError(error instanceof Error ? error.message : "Unable to update expense.");
    } finally {
  ...
  async function handleDeleteExpense(expense: ExpenseItem) {
    if (!token) return;
    const confirmed = window.confirm(`Delete ${expense.category} expense for ${currency(expense.amount)}?`);
    ...
    try {
      await deleteExpense({ token, id: expense._id }).unwrap();
      setExpenseError("");
      setOpenExpenseId((currentId) => (currentId === expense._id ? null : currentId));
      setEditingExpenseId((currentId) => (currentId === expense._id ? null : currentId));
    } catch (error) {
      setExpenseError(error instanceof Error ? error.message : "Unable to delete expense.");
    }
  }
    ...
    <div className="min-w-0 space-y-5 md:space-y-6">
      <div className="flex flex-col gap-4 rounded-[1.5rem] bg-slate-900 px-4 py-5 text-white shadow-[0_30px_80px_rgba(15,23,42,0.2)] sm:rounded-[2rem] sm:px-6 sm:py-6 md:flex-row md:items-end md:justify-between">
        <div className="min-w-0">
          <p className="text-xl uppercase tracking-[0.1em] text-teal-300 sm:text-2xl">Expenses</p>
          <p className="mt-2 max-w-2xl text-sm text-slate-300">
            Expand any item to review description, transaction date, receipt details, and more.
          </p>
        </div>
        <Link
          href="/dashboard"
      ...
      <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm sm:rounded-[1.75rem] sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            {expenseError ? <p className="mt-2 text-sm text-rose-600">{expenseError}</p> : null}
          </div>

          <div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-3 lg:max-w-2xl">
            <label className="grid gap-2">
              <span className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500">Category</span>
              <select
            ...
            >
              <div className="flex flex-col gap-4 px-4 py-4 sm:px-5 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap md:justify-end">
                  ...
                  <button
                    ...
                    aria-label={isOpen ? "Hide expense details" : "View expense details"}
              ...
              {isOpen ? (
                <div className="border-t border-slate-100 px-4 py-5 sm:px-5">
                  {isEditing ? (
                    <form
                      onSubmit={(event) => handleUpdateExpense(event, expense)}
                    ...
                    >
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <label className="grid gap-2">
                          ...
                          <input
                            name="date"
                            type="date"
                            defaultValue={formatDateInputValue(expense.date)}
                            className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none"
                            required
                          />
                        </label>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button
                          type="submit"
                          disabled={updateExpenseMutation.isLoading || uploadingExpenseId === expense._id}
                        ...
                        >
                          {uploadingExpenseId === expense._id
                            ? "Uploading..."
                  ...
                  <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(240px,280px)]">
                    <div className="grid gap-4">
                      ...
                      <DetailRow label="Amount" value={currency(expense.amount)} />
                      <DetailRow label="Description" value={expense.description || "No description provided"} />
                      <DetailRow label="Transaction date" value={new Date(expense.date).toLocaleString()} />
                    ...
                    <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4">
                      ...
                      {expense.receiptUrl ? (
                        <div className="mt-3 space-y-3">
                          {isImageFileUrl(expense.receiptUrl) ? (
                            <Image
                        ...
                        <p className="mt-3 text-sm text-slate-500">No receipt or document uploaded for this expense.</p>
                      )}
                    </div>
                  </div>
                </div>
